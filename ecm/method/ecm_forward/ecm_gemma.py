...
from ecm.util.cache_utils import apply_rotary_pos_emb_single_withpos, CompressCache, apply_rotary_pos_emb_single
import types
import logging
...
def enable_gemma_ecm_attention(model):
    model.model.forward = types.MethodType(GemmaModel_forward, model.model)
    enable_gemma_ecm_attention_recursive(model)

def enable_gemma_ecm_attention_recursive(model):
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            enable_gemma_ecm_attention_recursive(module)
