...
from ecm.util.cache_utils import apply_rotary_pos_emb_single_withpos, CompressCache
...
def enable_qwen2_ecm_attention_recursive(model):
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            enable_qwen2_ecm_attention_recursive(
                module,
            )
...
def enable_qwen2_ecm_attention(model):
    model.model.forward = types.MethodType(Qwen2Model_forward, model.model)
    enable_qwen2_ecm_attention_recursive(model)
