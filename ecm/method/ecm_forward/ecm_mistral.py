...
from ecm.util.cache_utils import apply_rotary_pos_emb_single, CompressCache
import types
import logging
...
def enable_mistral_ecm_attention_recursive(model):
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            enable_mistral_ecm_attention_recursive(
                module,
            )
...
def enable_mistral_ecm_attention(model):
    model.model.forward = types.MethodType(MistralModel_forward, model.model)
    enable_mistral_ecm_attention_recursive(model)
